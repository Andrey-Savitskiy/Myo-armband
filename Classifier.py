...
class Classifier(object):
	...
	def __init__(self, user_name: str, name="Classifier", color=(0,200,0)):
		# Add some identifiers to the classifier to identify what model was used in different screenshots
		self.name = name
		self.color = color
		self.path = os.path.abspath(f'data/{user_name}')

		if not os.path.exists(self.path):
			os.makedirs(self.path)

		for i in range(10):
			with open(self._get_path_(i), 'ab') as f: pass
		self.read_data()

	def _get_path_(self, number: int) -> str:
		return f'{self.path}/vals{number}.dat'

	def store_data(self, cls, vals):
		with open(self._get_path_(cls), 'ab') as f:
	...
	def read_data(self):
		...
		Y = []
		for i in range(10):
			X.append(np.fromfile(self._get_path_(i), dtype=np.uint16).reshape((-1, 8)))
	...
	def delete_data(self):
		for i in range(10):
			with open(self._get_path_(i), 'wb') as f: pass
		self.read_data()
...
class MyoClassifier(Myo):
	...
	def run_gui(self, hnd, scr, font, w, h):
		...
		for i in range(10):
			...
			clr = (0, 255, 0) if i == r else (255,255,255)
...
class Live_Classifier(Classifier):
	...
	'''
	def __init__(self, classifier, user_name, name="Live Classifier", color=(0,55,175)):
		self.model = classifier
		Classifier.__init__(self, name=name, color=color, user_name=user_name)
